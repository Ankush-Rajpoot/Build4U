...
const processPayment = async (paymentRequest) => {
  try {
    ...
    const platformFeeAmount = cashfreeService.calculatePlatformFee(paymentRequest.amount);
    const platformFeePercentage = parseFloat(process.env.PLATFORM_FEE_PERCENTAGE) || 5;
    const workerAmount = paymentRequest.amount - platformFeeAmount;

    // Generate unique transaction ID
    const transactionId = cashfreeService.generateTransferId();

    // Calculate baseAmount (assume no tax for now, or set as amount minus platform fee)
    ...
    else if (month >= 1 && month <= 3) quarter = 'Q4';
    ...
    const transaction = new Transaction({
      ...
      type: 'payment', // must match enum in Transaction schema
      taxDetails: {
        isApplicable: false,
        totalTaxAmount: 0
      }
    ...
    // Automatically create an invoice after successful payment
    // Import invoiceService at the top if not already imported
    const invoiceService = (await import('../services/invoiceService.js')).default;
    await invoiceService.createInvoice(paymentRequest._id, {
      createdBy: paymentRequest.client,
